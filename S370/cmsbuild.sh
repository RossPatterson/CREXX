...
# Start Hercules.
GITDIR=`pwd`
. /usr/local/hercules380/setup.sh
# (cd /opt/hercules/vm370 ; hercules -f $GITDIR/hercules.conf >/dev/null 2>/dev/null -d) &
(cd /opt/hercules/vm370 ; hercules -f $GITDIR/hercules.conf -d) &
...
# Logon CMSUSER.
herccontrol "/cp disc" -w "^VM/380 Online"
...
# herccontrol "/cmsmake" -w "^Ready;" -t 250
  herccontrol "/cmsmake" -w "^Ready"  -t 250 # It doesn't pass yet.
...
# Logoff.
herccontrol "/logoff" -w "^VM/380 Online"
